 ...
 *
 * <ul>
 *     <li>Dans celle-ci on va retrouver :</li>
 *     <ul>
 *          <li>la position du joueur</li>
...
public class Joueur {
    ...
    public static final int TAILLE_NOM_MAX = 15;
    ...
    /**
     * contient le nom du joueur dans la limite de 4 a 15 caracteres
     */
    private String nom;
    ...
    /**
     * Nouvel objet joueur
     * @param position fournit la position du joueur parmis les 4 positions possibles
    ...
    public Joueur(int position, String nom) {
        ...
        if (!(position == 0 || position == 1 || position == 2 || position == 3)) {
            throw new IllegalArgumentException("Erreur de saisie de la position qui " +
                    "doit être compris entre " + POSITION_MIN + " et " + POSITION_MAX +
                    ": " + toString());
        }
        ...
        if (nom.length() >= TAILLE_NOM_MAX || nom.length() <= TAILLE_NOM_MIN) {
            throw new IllegalArgumentException("Erreur de saisie de du nom qui " +
                            "doit être compris entre " + TAILLE_NOM_MIN + " et " +
                            TAILLE_NOM_MAX + " charactères : " + toString());
        }
        this.nom = nom;
    ...
    /**
     * creer une main a partir d'un tableau mainAEntrer et le mets dans un tableau mainJoueur
    ...
    public void enleverCarte(Carte carteAEnlever) {
        ...
        /* Analyse la main du joueur et verifie si la carte est dedans pour l'enlevée */
        for(int k = 0; k < mainDuJoueur.length ; k++){
            if (mainDuJoueur[k].equals(carteAEnlever)) {
        ...
        /* le boolean reste sur false si la carte n'a pas été trouvé et provoque une
        *  RuntimeException */
        if (!carteTrouve){
            throw new RuntimeException("Erreur : la carte n'a pas été trouvée dans la main ");
        }
     ...
     *         Trèfle.</li>
     *         <li>Pour les autres joueurs ils doivent fournir à la couleur
    ...
    public Carte[] cartePossible(Carte[] centreTable, int numPlis,
        ...
        /* Si c'est le premier tour et premier carte a poser */
    ...
    public void ajouterCarte(Carte carteAAjouter) {
        ...
        if(!estAjoute) {
            throw new RuntimeException("La carte n'a pas pu etre ajoutée");
        }
    }
